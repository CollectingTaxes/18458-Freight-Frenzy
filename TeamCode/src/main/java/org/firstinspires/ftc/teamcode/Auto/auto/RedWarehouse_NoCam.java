...
@Autonomous(name="RedWarehouse_NoCam", group="Roadrunner Paths")
public class RedWarehouse_NoCam extends LinearOpMode {
    @Override
    public void runOpMode() {
        ...
        double armHeight = 0;
        double hubDistance = 25;

        // move camera
        //robot.Cam.setPosition(auto.CAMERA_RED_CAROUSEL_POS);
        //robot.Arm.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);

        //RedWarehouseDuckDetect detector = new RedWarehouseDuckDetect();
        ...
        /*if (detector.getAnalysis() == RedWarehouseDuckDetect.DuckPosition.RIGHT) {
            armHeight = auto.Arm_High;
            hubDistance = auto.Arm_High_dis;
        }

        if (detector.getAnalysis() == RedWarehouseDuckDetect.DuckPosition.CENTER) {
            armHeight = auto.Arm_Mid;
            hubDistance = auto.Arm_Mid_dis;
        }

        if (detector.getAnalysis() == RedWarehouseDuckDetect.DuckPosition.LEFT) {
