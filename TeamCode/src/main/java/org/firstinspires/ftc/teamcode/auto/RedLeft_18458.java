...
import org.firstinspires.ftc.teamcode.auto.RedLeftCam_18458;
...
@Autonomous(name = "RedLeft_18458", group = "Linear Opmode")
public class RedLeft_18458 extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        hardware.Cam.setPosition(var.REDLEFTCAM);

        RedLeftCam_18458 detector = new RedLeftCam_18458();
        ...
        if (detector.getAnalysis() == RedLeftCam_18458.ShippingPos.LEFT) {
        ...
        if (detector.getAnalysis() == RedLeftCam_18458.ShippingPos.CENTER); {
            hardware.Arm.setTargetPosition((int) var.ARMMID);
            hardware.Arm.setMode(DcMotorEx.RunMode.RUN_TO_POSITION);
            sleep(100);
            robot.encoderDrive(DRIVE_SPEED,  48,  48, 48, 48, 5.0);
        ...
        if (detector.getAnalysis() == RedLeftCam_18458.ShippingPos.RIGHT); {
            hardware.Arm.setTargetPosition((int) var.ARMLOW);
            hardware.Arm.setMode(DcMotorEx.RunMode.RUN_TO_POSITION);
            sleep(100);
            robot.encoderDrive(DRIVE_SPEED,  48,  48, 48, 48, 5.0);
